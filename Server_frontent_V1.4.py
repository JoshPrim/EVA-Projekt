...
elevatorStateCountACTIVE = getDesiredState(listWithStates, 'ACTIVE')
elevatorStateCountINACTIVE = getDesiredState(listWithStates, 'INACTIVE')
elevatorStateCountUNKNOWN = getDesiredState(listWithStates, 'UNKNOWN')
...
escalatorStateCountACTIVE = getDesiredState(listWithStates, 'ACTIVE')
escalatorStateCountINACTIVE = getDesiredState(listWithStates, 'INACTIVE')
escalatorStateCountUNKNOWN = getDesiredState(listWithStates, 'UNKNOWN')
...
elevator_key_array = []
elevator_value_array = []

for key, value in dictStateExplanationReason.items():
    elevator_key_array.append(key)
    elevator_value_array.append(value)
...
escalator_key_array = []
escalator_value_array = []

for key, value in dictStateExplanationReason.items():
    escalator_key_array.append(key)
    escalator_value_array.append(value)


###############################################
##### Stammdaten einlesen und verarbeiten #####
###############################################
...
active = elevators[elevators['state'] == 'ACTIVE']
...
escalator = facilities.aggregate([
...
escalator = pd.DataFrame(list(escalator))
escalator.columns = ['equipmentnumber', 'description', 'geocoordX', 'geocoordY', 'lastStateChangeDate', 'state']

escalator_inactive = escalator[escalator['state'] == 'INACTIVE']
escalator_active = escalator[escalator['state'] == 'ACTIVE']

# Zoom am ausgewählten Ort
escalator_geolocator = Nominatim(user_agent="Eva_Dashboard")
...
test = facilities.aggregate([
    ...
    {'$project':{
        ...
        {'$filter' : {
           'input' : '$datetime',
           'cond' : { '$gte' : [ "$datetime", {'$subtract' : [ {'$last' : '$datetime'}, datetime.datetime.now()]} ]}
        }}
        }},
...
page_rolltreppen = html.Div(children=[
    ...
    html.Div([
        ...
        html.Div([
            ...
            figure={
                'data': [
                    {'x': ['aktiv', 'inaktiv', 'keine Information'], 'y': [escalatorStateCountACTIVE, escalatorStateCountINACTIVE, escalatorStateCountUNKNOWN], 'type': 'bar', 'name': 'Rolltreppen',
        ...
        html.Div([dcc.Graph(
            figure=go.Figure(
                ...
                id='escalator_mid_graph'
                    )], style={'width': '60%', 'text-align': 'left', 'display': 'inline-block', 'padding-top': 10, 'padding-left': 140, 'padding-bottom': 10 }),
        ...
        html.Div([
            ...
            ], style={'width': '60%', 'text-align': 'left', 'display': 'inline-block', 'padding-top': 10, 'padding-left': 140, 'padding-bottom': 10 }), ## neu vorher gar nichts

            #linker Teil ########################################## geändert alle ids + escalator
            html.Div([
                html.Div(['Stadt:  '], style={'margin-left': 'auto', 'margin-right': 'auto', 'display': 'inline-block'}),
                dcc.Input(id='escalator_stadt_input', value='Frankfurt', type='text', style={'margin-left': '5', 'margin-right': 'auto', 'display': 'inline-block'}),
                html.Div(['Bundesland:  '], style={'margin-left': '15', 'margin-right': 'auto', 'display': 'inline-block'}),
                dcc.Input(id='escalator_bundesland_input', value='Hessen', type='text', style={'margin-left': '5', 'margin-right': 'auto', 'display': 'inline-block'}),
                html.Br(), html.Br(),
                dcc.RadioItems(
                ...
                html.Iframe(id='escalator_karte', srcDoc=open('map_inactive_elivators.html', 'r').read(),
                            style={'width': '90%', 'height': '30em'})
            ], style={'width': '60%', 'text-align': 'left', 'display': 'inline-block', 'padding-top': 10, 'padding-left': 140, 'padding-bottom': 10 }),#style={'width': '49%', 'display': 'inline-block'}),
